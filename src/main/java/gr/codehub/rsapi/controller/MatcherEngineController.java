...
public class MatcherEngineController {
    ...
    @PostMapping("automatic/{jobOfferId}")
    public List<Match> automaticMatch(@PathVariable long jobOfferId) throws JobOfferNotFoundException, JobOfferAlreadyClosed {
    ...
    @PostMapping("partial/{jobOfferId}")
    public List<Match> partiallyMatch(@PathVariable long jobOfferId) throws JobOfferNotFoundException, JobOfferAlreadyClosed {
        log.info("\nPOST REQUEST: Calling addPartiallyMatch method ");
        return matchService.addPartiallyMatch(jobOfferId);
    }
